...
async function fetchSourceContent(url, timeout = 30000) {
  const response = await axios.get(url, {
    ...
    maxRedirects: 5,
    headers: {
      'User-Agent': 'Gudnayber article generator'
    },
    validateStatus: status => status >= 200 && status < 400
  });
...
function generateImagePrompt(topic) {
  return {
    prompt: `Create a sophisticated editorial illustration for the Gudnayber article "${topic.title}" about ${topic.subject}, in the ${topic.category} category. Visualize the specific human idea, relationship, tension, or real-world cooperation at the heart of this topic—not a generic city scene. Use a strict two-color duotone palette only: Gudnayber brick red #B44334 and deep navy blue #071B4C. Use expressive engraved line work, bold screen-printed shapes, subtle paper grain, and compassionate human gestures. Incorporate a few understated motifs from the Gudnayber hero pattern—olive branches, a dove, a simple fish, a loaf of bread, or the interlocking heart mark—as small symbolic details, a border, or background texture; select only motifs that meaningfully support the topic and never crowd the central scene. Respect human dignity and agency. Hopeful but unsentimental, editorial rather than promotional. No third color, no black, no white background, no gradients, no photorealism, no logos other than a subtle Gudnayber heart motif, no readable text, no typography, no watermark, no partisan propaganda, no religious caricatures, no stereotypes, no staged poverty, and no exploitative depiction of vulnerable people.`,
    negative_prompt: `third color, black, white background, grayscale, full color, gradient, photorealistic, stock photo, generic cityscape, excessive symbols, repeated logo wall, readable text, typography, watermark, partisan propaganda, religious caricature, stereotype, savior imagery, staged poverty, exploitative imagery, identifiable vulnerable person`
  };
}
